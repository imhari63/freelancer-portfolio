  ...
  return (
    <article className="bg-gradient-to-br from-[#07102a] to-[#08102a] rounded-xl overflow-hidden shadow-lg hover:scale-[1.02] transition transform">
      <div className="relative h-48 md:h-56 lg:h-44">
        {hasImage ? (
          <Image src={project.image?.src || project.img || '/png/placeholder.png'} alt={project.name} fill className="object-cover cursor-pointer" />
        ) : hasVideo ? (
...
export default function PortfolioShowcase(){
  ...
  const items = useMemo(() => {
    ...
    if(filter === 'images') return projectsData.filter(p => (typeof p.image?.src === 'string' && p.image?.src.length > 0) || (typeof p.img === 'string' && p.img.length > 0))
          ...
          <nav className="flex gap-2">
            {filters.map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-3 py-1 rounded-full text-sm cursor-pointer focus-visible:ring-2 focus-visible:ring-violet-400 hover:bg-violet-400 hover:text-white transition-colors duration-150 ${filter===f? 'bg-violet-600 text-white' : 'bg-white/5 text-gray-200'}`}
